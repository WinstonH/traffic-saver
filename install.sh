...
============================================================';
http='https://';
host='raw.githubusercontent.com/winstonh/traffic-saver/master/';
vpnfile=lib/;
sq=squid.conf;
squser=auth_user;
css=errorpage.css;
...
function authentication() {
    echo -n -e "请输入Github官方网址 [\033[32m $key \033[0m] ："
    read PASSWD
    readkey=$PASSWD
    if [[ ${readkey%%\ *} == $key ]]
    then
		...
		echo -e '\033[33m		Powered by Github.com 2018\033[0m'
		echo -e '\033[33m		All Rights Reserved \033[0m'
		echo -e '\033[33m	官方网址：https://github.com/ \033[0m'
		echo -e '\033[33m	                                       欢迎你的加入！\033[0m'
...
echo "	#################################################
	...
	verb 3
	reneg-sec 0
	#github.com" >/etc/openvpn/server.conf
echo "	#################################################
	#               vpn流量控制配置文件             #
	#                             by：云免流控™     #
	#                                2017-03-08     #
	...
	verb 3
	reneg-sec 0
    #github.com" >/etc/openvpn/server-udp.conf
...
echo -e '\033[34m                官方网址：https://github.com\033[0m'
...
echo
echo -e '\033[33m请输入正版密钥开启安装向导（购买网址:\033[32m github.com \033[0m）'
echo
